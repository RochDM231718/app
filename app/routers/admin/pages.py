...
from sqlalchemy import or_, asc, desc  # <-- Добавлены asc, desc
...
async def search_documents(
    ...
    if status:
        base_query = base_query.filter(Achievement.status == status)
...
async def index(
        ...
        query: Optional[str] = "",
        status: Optional[str] = None,
        sort: Optional[str] = "created_at",  # Параметр сортировки
        order: Optional[str] = "desc",  # Порядок
        db: Session = Depends(get_db)
):
    check_access(request)
    ...
    if hasattr(Achievement, sort):
        ...
        # Если поле не найдено (или это поле User), сортируем по дате
...
async def delete_document(
    ...
    # Создаем менеджер без аргументов
    translator = TranslationManager()

    # Передаем локаль в метод gettext
